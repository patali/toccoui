...
def action_launch_plugin(menu, w, args):
    name, plugin = args
    win = MTInnerWindow(size=(1024,768), pos=w.pos)
...
if __name__ == '__main__':
    ...
    layme = MTBoxLayout(padding=10, spacing=10, color=(0,0,0,1.0),pos=(75,300))
    w.add_widget(layme)
    ...
    while len(plist):
        ...
        icon = MTicon(filename = infos.get('icon'),scale=0.6,label=infos.get('title'),action=action_launch_plugin, args=[name, plugin])
